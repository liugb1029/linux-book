...
- 目录项，简称为 dentry，用来记录文件的名字、索引节点指针以及与其他目录项的关联关系(父目录、子目录)。多个关联的目录项，就构成了文件系统的目录结构。不过，不同于索引节点，目录项是由内核维护的一个内存数据结构，所以通常也被叫做目录项缓存。
...
### 硬链接
硬链接(hard link) ： 可以将它理解为一个 “指向原始文件 inode 的指针”，系统不为它分配独立的 inode 和 文件。所以，硬链接文件与原始文件其实是同一个文件，只是名字不同。我们每添加一个硬链接，该文件的 innode 连接数就会增加 1 ； 而且只有当该文件的 inode 连接数为 0 时，才算彻底被将它删除。因此即便删除原始文件，依然可以通过硬链接文件来访问。**需要注意的是，我们不能跨分区对文件进行链接，不能对目录文件进行创建硬链接**
硬链接特点：
...
举例： 磁盘上有个文件
![](static/hardlink-example-01.jpeg)
此时新建一个硬链接
```bash
root@luke:~# ln filename NewFileName

```
![](static/hardlink.jpeg)

从上图中可以看出，fileName 和 newFileName 都指向同一个文件，因此，无论你编辑哪一个文件，另一个文件也会做相应的改变。比如:
```bash
root@luke:~# echo 'helloworld' >> filename
root@luke:~# cat NewFileName
helloworld // 此时原文件中也多出了 helloWorld。
# 文件的inode links链接计数变为2。
root@luke:~# stat filename
  File: filename
  Size: 11        	Blocks: 8          IO Block: 4096   regular file
Device: fd00h/64768d	Inode: 326970      Links: 2
Access: (0644/-rw-r--r--)  Uid: (    0/    root)   Gid: (    0/    root)
Access: 2021-01-04 07:41:30.039547580 +0000
Modify: 2021-01-04 07:41:27.299517223 +0000
Change: 2021-01-04 07:41:27.299517223 +0000
 Birth: -
root@luke:~#
```
删除其中任意一个文件，另外一个文件的内容还存在。

![](static/hardlink-delete-01.png)

![](static/hardlink-delete-02.jpeg)


### 软链接(符号链接)
软链接有自己的索引节点(即inode号)和用户数据块(datablock)，但是用户数据块存储的内容是另一文件的路径名的指向。软链接其实就是一个普通文件，只是数据块内容有点特殊。
软链接特点：
1. 软链接有自己的文件属性及权限等；
2. 可对不存在的文件或目录创建软链接；
3. 软链接可交叉文件系统；
...
一个空目录的链接计数为2，这是由于目录文件本身以及其下的'.'文件。
```bash
root@luke:~# stat test
  File: test
  Size: 4096      	Blocks: 8          IO Block: 4096   directory
Device: fd00h/64768d	Inode: 329609      Links: 2
Access: (0755/drwxr-xr-x)  Uid: (    0/    root)   Gid: (    0/    root)
Access: 2021-01-04 07:58:58.061774330 +0000
Modify: 2020-12-20 03:28:19.469698814 +0000
Change: 2020-12-20 03:28:19.469698814 +0000
 Birth: -
root@luke:~# ls -id test test/.
329609 test  329609 test/.
```

### 查找软链接与硬链接
```bash
# 查找在路径 /home 下的文件 data.txt 的软链接
root@luke:~# find /home -lname data.txt 
/home/harris/debug/test2/a 
 
# 查看路径 /home 有相同 inode 的所有硬链接
root@luke:~# find /home -samefile /home/harris/debug/test3/old.file 
/home/harris/debug/test3/hard.link 
/home/harris/debug/test3/old.file 
 
root@luke:~# find /home -inum 660650 
/home/harris/debug/test3/hard.link 
/home/harris/debug/test3/old.file 
 
// 列出路径 /home/harris/debug/ 下的所有软链接文件
root@luke:~# find /home/harris/debug/ -type l -ls 
